...
Tournament mode (a game MODE, not a mini-game): the host checks ≥2 mini-games in the lobby (`GamePicker` tournament card) + two toggles, then `tournament:start`. The server (RoomManager `Tournament` state + orchestration in `apps/server/src/index.ts`) chains the chosen games in fixed order (blackjack → roulette → poker), each as a "leg". A leg ends either after `roundsPerLeg` rounds (toggle "Limiter par manches" ON → chip leader wins) OR by elimination when only one player can still play (toggle OFF). Optional "Mises croissantes" toggle (`escalate`): each time a player is knocked out, the min bet (poker: blinds) rises to base × (eliminated+1) — engines expose `setMinBet`/`setBlinds`, applied via `tournamentAfterRound` which also detects eliminations (chips below the playable floor) and the elimination leg-end. Rebuy is disabled during tournaments (going broke = out of the current leg; reset next leg). Roulette's `maybeSpin` ignores broke players so they don't block the wheel. The leg winner / point goes to the chip leader (`endTournamentLeg`), then a 5s intermission (`INTERMISSION_MS`) before the next leg auto-starts (`startTournamentLeg`); after the last leg the tournament is `done`. Points are keyed by socket id. Broadcast on a separate `tournament:state` channel (TournamentView: games, legIndex, phase playing/intermission/done, standings, lastWinners) ALONGSIDE the leg's normal `game:state`; `RoomState.tournamentActive` keeps clients in the in-game view through intermissions. Client: `Tournament.tsx` renders a `.tournament-banner` (leg pills + progress) above the table during play, and a `.tournament-intermission` / `.tournament-end` overlay (standings, champion, host "Retour au lobby" → `game:end`) otherwise. Solo works for blackjack/roulette legs; a poker leg needs ≥2 players. NOTE poker chips are public so `gamePlayerChips` reads them from `getViewFor("")`.
